...
class BoardPacket:
    ...
    def white_board(self):
        return [[j for j in i] for i in self.__board[::-1]]
    @property
    def black_board(self):
        return [[j for j in i[::-1]] for i in self.__board]
    ...
    def __dict__(self):
        return {
            "board":self.white_board if self.__perspective else self.black_board,
